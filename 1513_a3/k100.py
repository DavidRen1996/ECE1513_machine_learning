...
def k_means(K,data):
    np.random.seed(421)
    u=tf.Variable(tf.random.truncated_normal([K,100],0.0,stddev=1.0,dtype=tf.float32))
    u_expand=tf.expand_dims(u,0)#expand dimension to substract
    x=tf.placeholder(tf.float32,shape=[10000,100],name='x')
    ...
    with tf.Session() as sess:
        ...
        for index in range(500):
            ...
            if index==499:
                ...
                reshaped_final_center=np.reshape(final_center,[K,100])
                for i in range(K):
                    num=0
